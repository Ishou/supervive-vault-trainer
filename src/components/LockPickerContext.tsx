...
function lockPickerReducer(
  ...
  switch (action.type) {
    ...
    case "toggleRunning": {
      const { difficulty, autoDifficulty, options, result, running, offset } =
        state;
      const { cursorOffset, cursorSize, perfectOffset, perfectSize } = action;
      if (result === "start") {
        ...
        if (compareRange(cursorRange, range(perfectOffset, perfectSize)))
          return { ...state, running: false, result: "perfect" };
        else if (compareRange(cursorRange, range(offset, options.size)))
          return { ...state, running: false, result: "ok" };

        return { ...state, running: false, result: "fail" };
      }
      ...
      return {
        ...
        options: !autoDifficulty ? state.options : difficulties[newDifficulty],
    ...
    case "newFrame": {
      ...
      return {
        ...
        cursor:
          state.cursor +
          (!state.running ? 0 : action.deltaTime * 0.36 * state.options.speed),
        fpsCounter:
          lasts.length < 30
